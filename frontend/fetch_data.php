...
try {
    // Check if a station ID is provided
    if (!isset($_GET['station_id'])) {
        throw new Exception('No station ID provided');
    }

    $email = $_GET['station_id'];
    ...
    if (!$result) {
        throw new Exception('Table does not exist or could not be described');
    }
    ...
    echo json_encode(['error' => $e->getMessage(), 'columns' => [], 'data' => []]);
}
